...
for (i in 2:length(args)) {
    ...
    data <- data[432:nrow(data),] # just TFs
...
# plot auprc
auprc_file <- paste(out_prefix, ".encode_auprc.TFs.pdf", sep="")
plot_metric(auprc_results, auprc_file, "ENCODE/Roadmap\nDNase peaks", "AUPRC", c(0.0,1.0))

# auroc
auroc_file <- paste(out_prefix, ".encode_auroc.TFs.pdf", sep="")
plot_metric(auroc_results, auroc_file, "ENCODE/Roadmap\nDNase peaks", "AUROC", c(0.5,1.0))

# recall
recall_file <- paste(out_prefix, ".encode_recall.TFs.pdf", sep="")
plot_metric(recall_results, recall_file, "ENCODE/Roadmap\nDNase peaks", "Recall at 25% FDR", c(0,1.0))
