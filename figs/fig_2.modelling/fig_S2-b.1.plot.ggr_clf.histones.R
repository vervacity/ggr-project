...
# auprc
auprc_file <- paste(out_prefix, ".ggr_auprc.histones.pdf", sep="")
auprc_results$train <- factor(auprc_results$train, levels=c(rand_init, pretrain))
plot_metric(auprc_results, auprc_file, "Keratinocyte \nATAC peaks", "AUPRC", c(0.0, 0.6))

# auroc
auroc_file <- paste(out_prefix, ".ggr_auroc.histones.pdf", sep="")
