...
# code
SCRIPT_DIR=~/git/ggr-project/figs/fig_SX.homotypic

# dirs
GGR_DIR=/mnt/lab_data/kundaje/users/dskim89/ggr/integrative/v1.0.0a
INFER_DIR=/mnt/lab_data/kundaje/users/dskim89/ggr/nn/inference.2019-10-08

# sig pwms
SIG_PWMS_FILE=$INFER_DIR/motifs.sig/motifs.adjust.diff.rna_filt/summary/ggr.pwms_patterns_summary.txt
...
# first, check all sig motifs in DYNAMIC regions for multiplicity
# only use regions that pass a cutoff
# use mut results or no? <- no, since only mutated strongest hit. 
# TODO have this produce a pwm sig file for downstream
$SCRIPT_DIR/analyze.genome.multiplicity.py genome.multiplicity $SIG_PWMS_FILE $LINKS_FILE $TSS_BED $BACKGROUND_GENES_FILE $EARLY_MOTIFS $MID_MOTIFS $LATE_MOTIFS
