...
for (plot_i in 1:length(plots)) {
    ...
    title <- paste(ids[2], "/", ids[3], " motifs\n", ids[2], " ChIP (", ids[4], ")", sep="")
    
    # get subset and set y_lim
    data_subset <- data[data$plot == plot,]
    data_subset <- data_subset[!is.na(data_subset$value),]

    # manually set y_max and levels
    if (grepl("day0", plot)) {
        y_max <- 0.010
        levels <- unique(data_subset$var)
        data_subset$var <- factor(data_subset$var, levels=levels)
    } else {
        y_max <- 0.025
    }

    # plot
    ggplot(data_subset, aes(x=reporter, y=value, group=var, fill=var)) +
        geom_bar(
            colour=my_color, size=0.230, width=0.55,
            stat="summary", fun="mean", position=position_dodge()) +
        stat_summary(
            geom="errorbar",
            fun.data=mean_se,
            #fun.data=mean_sdl, fun.args=list(mult=1),
            colour=my_color, size=0.230, width=0.45,
